# ==============================================================================
# UltraChat S2S - Gemini ASR + LLM + Dia TTS Integration
#
# This version integrates Google Gemini for ASR/LLM and Dia TTS for speech synthesis.
# This is a fundamental shift from previous models, leveraging cloud APIs.
#
# REQUIRED:
# 1. Google API Key set as environment variable: export GOOGLE_API_KEY='YOUR_API_KEY'
# 2. Installation of `google-generativeai` and `dia-tts` packages.
#
# ARCHITECTURE:
# - VAD (Ears): Silero VAD (Lightweight, on-device)
# - ASR+LLM (Brain): Google Gemini API (Cloud-based, powerful)
# - TTS (Mouth): Dia TTS (Advanced, expressive speech synthesis)
#
# This is a significant upgrade, offering robust speech understanding and high-quality,
...
# TTS Model (Kyutai/Moshi was replaced with Dia TTS based on your request)
# Ensure you have installed 'dia-tts' and its dependencies.
# NOTE: Dia TTS might have specific installation instructions or model downloads.
# This code assumes a standard pip install and direct API calls.
# If the import fails, check Dia TTS's official GitHub for installation.
from dia_tts.inference.tts import TTS as DiaTTS # Renamed to avoid conflict with potential local TTS models
# The 'tts_model' variable will be an instance of DiaTTS
...
ultravox_pipe = None # We are not using Ultravox anymore, but keeping variable name for now.
gemini_model = None # Will hold the Gemini model instance
tts_model = None
vad_model = None
...
<script>
    ...
    async function start() {
        ...
        try {
            // No need to explicitly get user media here if the backend handles it via WebRTC offer/answer
            // The 'start' button's primary role is to initiate the WebSocket and WebRTC connection.
            
            pc = new RTCPeerConnection({ iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] });
            
            // Add local audio track only if we are initiating the stream from the client.
            // Since Gemini handles ASR, we don't need to add our microphone track to the PC here
            // unless we were doing direct client-to-client communication without a backend.
            // The flow is: Client Audio -> WebSocket -> Backend (Gemini ASR) -> Backend (Gemini LLM) -> Backend (Dia TTS) -> WebSocket -> Client Audio Output
                ...
                if (event.track.kind === 'audio') {
                    if (remoteAudio.srcObject !== event.streams[0]) {
                        ...
                        try {
                            ...
                            console.error("Autoplay failed or error during playback:", error);
                            updateStatus('❌ Audio Error. May need user interaction.', 'disconnected');
                            // Consider adding a "Play Audio" button for users if autoplay fails
                        }
                    }
                }
            };
            ...
            ws.onmessage = async e => {
                const data = JSON.JSON.parse(e.data);
            ...
            console.error('Error starting or connecting:', err);
    ...
    def _blocking_asr_llm_tts(self, audio_array) -> np.ndarray:
        try:
            ...
            with torch.inference_mode():
                ...
                results_list = tts_model.generate(entries, [condition_attributes]) # Pass as list
